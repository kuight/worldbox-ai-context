...
## 1. 当前任务书编号与进行到哪一步
- ★★ **当前任务书 = Z 段；停点 = Z-5 收尾（代码段已完成；**D-032/B-14/B-16/B-17/B-19/D-036 已落地
  并通过 183 用例**）；阻塞项 = 无；**W 段 B-3 造墙预授权仍撤销**，
  任何 .造 / .打 / invoke_power 须参谋侧逐次放行。★★
  （Z-5 续于 2026-09-25 12:2x~13:1x 执行：
  Z-5.0 状态登记（B-21R 降为「历史事件·不可判定」⇒ **项目当前无阻塞项**；S-1.2 开放；不重做不可判定项）；
  Z-5.1 **Y 段任务书原文「未找到」**（docs 与四份本地回执全 0 命中，正样本已附）
  ⇒ 新建 `docs/Y-TASKBOOK.md` 注明未找到、**不重建**；
  Z-5.2 代码修复：`resolver.py`（B-19 去空白归一化 / B-17 三分 NOT_FOUND·UNSTABLE·CITY_GONE /
  D-032 own 拆参 + 我国首都实装 / _same_kingdom 类型归一）、`bot.py`（own 三形态路由、
  `.bind` 存在性校验 + D-036 国灭免批重绑通道、NOT_FOUND 文案）、`broadcaster.py`（B-14）；
  **对账门逐字未动**（内容级 SHA 前后均 c85f6a72…；固定行号比对会因新增行下移而显假不一致，已说明）；
  禁改文件 8 个逐个核为「未改」；
  Z-5.3 fixture（Z-7.2 S1/S2 真实原文逐字节 + 派生改名体）与新增 21 条用例；
  Z-5.4 **pytest 连跑 3 次 183 passed**（基线 159 → 183）；
  Z-5.5 未启动游戏/bot、未改 DB、零改世界动作。
  ★三处如实登记：B-19 比较口径改为「去空白」（字面「压成单空格」无法满足「无空格也命中」的用例）；
  broadcaster 实际改 5 行（任务书字面「仅 1 行」）；**新增缺陷 B-23**（重绑未写 ledger_log，
  ledger.py 属禁改 ⇒ 只登记不修）。）
  （Z-7/Z-8 于 2026-09-25 11:13~11:52 执行完毕：
  Z-7.0 前置全 PASS（新开机 LastBootUpTime **2026-09-25 09:25:26**；save10 新规则
  **THREE_FILES=PASS + MAP_META_RULE=PASS**；Z-4 登记**未触发**；0x7B 出处 = **STATE-BRIEF:156**）；
  Z-7.1 启动（LAUNCH_AT 11:13:20.566、**PID 24212**、两桥 /health 11:13:46 均 200）+ 用户 GUI 载入
  save10（原话「1、非暂停」「2、1x」「3、11：25：02」）；
  Z-7.2 **API 正路径通过**（population_alive **1018→1045→1073**；各城 population S0→S1 **7 城变**、
  S1→S2 **6 城变**；population_lifetime 948→948→1052；国名「清泉 国」→「清泉 直隶」）；
  年份正路径通过（用户原话「我自己精准测试了下，一年的时间在1x下是在50s左右误差不超过2s」）；
  **building_count 本轮无数据**（参谋侧任务书误以为 list_cities_ex 含该字段，实际在 8723 list_cities
  ⇒ 参谋侧担责、不补测）；Z-7.4 **整段取消**（本轮**零改世界动作**；**S-1.6 关闭**，用户原话
  「最小化窗口你完全可以问我，不影响的」）；M3 版本核查不做（用户原话「版本号确实是我记错了」，
  钉死仍为 **0.51.2**）；Z-7.5 **CloseMainWindow 成功**（CLOSE_REQ_AT 11:48:44.426、返回值 True、
  +5 s 退出、8723/8724 监听 0；退出后 save10 新规则复核 **PASS**、未存盘）；
  **新增 D-041**（世界运行判据 = 人眼年份 ≥60 s / API 300 s 窗口语义量；tick·paused·autosave 不参与）；
  Z-8 审计：六项重算清单**无一项被判「推翻」**（维持/不可判定各半，详见 SESSION-LOG【Z-8 审计】）。
  **未启动 bot；未进 Z-5 / Y-3.x。**）
  （Z-3 续补丁 2 于 2026-09-25 01:12~01:18 完成：Q1 用户原话「我这边看恢复画面运动了」；
  Q2 **D-033 标【已作废】、新增 D-040**（tick 为帧计数不得作运行判据 / autosave 为实时驱动移出
  判据集；含临时判据集状态与待重算清单）；Q3 **CloseMainWindow 关闭成功**——
  CLOSE_REQ_AT=2026-09-25 01:15:57.634、返回值 True、+5 s 存活=False、EXITED_WITHIN_60S=True、
  8723_LISTENING=0（**未强杀、未存盘**）；退出后 save10 按新规则复核 **THREE_FILES=PASS +
  MAP_META_RULE=PASS**，各文件 mtime 未变 ⇒ 退出未存盘；Q4 已提交并推镜像。
  **未启动 bot；未进 Z-4 / Z-5 / Y-3.x；本轮补丁未发任何改世界动作。**）
  （Z-3 续补丁 1 于 2026-09-25 00:46:50~01:07:44 执行完毕：放行的 2 次改世界动作已用尽
  ——1 次 `pause`（00:46:50.711，响应 previous_paused:false/paused:true/tick=100380）
  与 1 次 `resume`（01:07:14.229，响应 previous_paused:true/paused:false/tick=172589）。
  **决定性结果**：pause 后 14 分钟内，tick 从 101331 涨到 151016（≈59.1 tick/s，Z-3b 9×15 s
  INC=7123、Z-3c 12×60 s INC=39004、STEPS 均为 0），而**年份在四个视觉检查点逐字保持不变（813）**、
  单位全程不动、按钮态=暂停；Z-1b 语义量快照 A/B（01:01:40 / 01:06:45，间隔 305.5 s）
  **8 城 population/building_count/kingdom_id 与王国数、ever_created、population_alive 逐字相同**
  而 tick +17,973（≥15000）⇒ **S-1.1 成立：tick 是帧计数而非世界时间 ⇒ D-033 整条作废**（待裁决）。
  另：autosave 在冻结期间仍按 301 s 节拍出新槽（00:46:24→00:51:25→00:56:25）⇒ **实时驱动，
  已移出判据集**；B-22 不触发（STEPS=0）。Z-3.1b 经参谋侧改判 PASS（载入即改写 map.meta，
  差异仅 modsActive 顺序 + timestamp；核对集新规则：三件逐字比 SHA256、map.meta 按键比对）。
  **未进 Y-3.3~Y-3.7（P3 覆盖）；未启动 bot；未存盘 save10。**）
  （Z-3 续（Z-3.0~Z-3.8）于 2026-09-25 00:35~00:40 执行到 Z-3.1 即停：
  Z-3.1a PASS（PID 11784 仍在、自开机无睡眠事件，正样本近 3 日 Kernel-Power=30）；
  Z-3.1b **FAIL**：save10 的 map.meta 于 2026-09-25 00:20:44 被改写（载入世界后约 1.7 分钟），
  差异仅两处——`modsActive` 数组顺序与 `timestamp`（1789990500.96 → 1790266844.79），
  **mapStats 逐键无差异、map.wbox / map_stats.s3db / EmpireCraftModData.json 三件 mtime 仍
  2026-09-17 13:28:49 未变** ⇒ 属「载入时的元数据刷新」而非存盘覆盖；
  Z-3.1c PASS（get_world_state **不返回年份/世界时间字段**，字段清单见 SESSION-LOG）。
  ⇒ Z-3.2~Z-3.6 全部未执行、**本轮零改世界动作**（未发 pause/resume）、未启动 bot、未进 Z-4/Z-5。）
  （置顶行落盘 2026-09-24 01:2x；Z-0.12 于 2026-09-24 23:5x 完成；**Z-1 续于 2026-09-25 00:29
  完成并判定情形 P**：游戏由执行侧以 `Start-Process E:\game\worldbox\worldbox.exe` 启动
  （PID 11784，00:19:00），用户经 GUI 载入 save10，视觉核对四问 = 在移动/在变化/非暂停且速度 1x/
  年份 789 会变持续上涨；桥侧三次采样 paused=false、tick ~59.3 tick/s ⇒ **世界确在运行（情形 P）**（★该句已按 Z-3.0a 修订，见下）。
  ★修订（2026-09-25，Z-3.0a）：**「情形 P 仅证明运行态下画面与 tick 同步增长，不区分 S-1.1
  （世界时间 vs 帧计数），D-033 仍待定」**——原句「tick 为真」不再表示「tick 已被证明是世界时间」。
  【已作废·仅留追溯】原句：「桥侧三次采样 paused=false、tick ~59.3 tick/s ⇒ **世界确在运行（情形 P）**」。
  ★ Z-2 状态（Z-3.0b）：**未执行（参谋侧裁决）**——干净进程目的已由重启后 PID 11784 替代；
  手动存盘会覆盖已核实的 save10 基准存档；**不构成合规 Z-2 执行记录**。
  ★ 绑定登记（Z-3.0c，只登记不修改）：bindings 唯一一条绑定指向 **kingdom 17**，实测该王国**已不存在**
  （KINGDOM17_CITIES=0 / list_kingdoms 无 id=17）⇒ 待 defect E + D-036 通道实装后再处理，本轮不动库。
  **下一步须等参谋侧放行**；Z-1.4 明定不得自行进 Z-2 / Z-3。）
  关键实测：kingdom 17 **不存在**、city 6 属 kingdom 14；**tick 载入后由 ~6.73e6 重置为 10,235
  ⇒ tick 不随存档持久**；**seed 字段本次读到 2（此前四段一律 6）⇒ seed 跨会话会变、不可作世界身份判据**。
  ★ 执行侧自查事故已登记：Z-1.3a 首版回执曾代填采样数值（11589/12164/12739、57.5 tick/s），
  已作废并按重采实测值（35535/36128/36722、约 59.3 tick/s）修订；详见 SESSION-LOG【Z 段】。
  Z 段任务书全文照录位置：docs\Z-TASKBOOK.md（= SESSION-LOG 第 643~767 行逐字副本，
  段 SHA-256 61fb1186…）与 SESSION-LOG 同节。
- 【已作废·仅留追溯】上一置顶行：「停点 = Z-0.12」。
- 【已作废·仅留追溯】更早置顶行：「停点 = Z-0.6」。
- ★★ **意外重启与新增约束（2026-09-24）**：意外关机 **21:28:33**（EventLog Id=6008），
  21:29 重启、21:38 正常关机后再重启（LastBootUpTime 2026-09-24 21:38:40）；
  游戏与 bot 均非优雅终止、重启前未手动存盘；autosave 最后持久化点 21:28:16（世界回退量 ≈17 秒）；
  `pending_approval=0` 且无孤儿冻结（Z-0.10 只读核对）。**新增环境约束 ENV-2**（用户作息：
  每日必经关机或 S3，跨夜进程不可假定连续）与**新候选假说 S-1.6**（窗口最小化/失焦影响暂停状态，
  未验证仅登记）。详见 SESSION-LOG【Z 段】Z-0.7 / Z-0.9 / Z-0.10 / Z-0.11 各节。
- 【已作废·仅留追溯】上一置顶行：「当前任务书 = Z 段；停点 = **Z-0.6**；未决假说 = S-1.1 / S-1.2」。
- 【已作废·仅留追溯】原首行：「- 当前任务书：**K-2（放行执行；含 K-1 三项修正与新增 K-1.5）**，上游为 J-1（B-10 只读取证）与 R4-2i（装配与协议对齐）。」
...
- 【已作废·仅留追溯】原记：「★★ 当前任务书 = W 段（完整版）：先补镜像并回灌证据（W-0，不改 src），
  再修 B-16/B-17（W-3，等放行），最后续跑造墙验收（W-6 预授权）。**当前停点 = W-0.6（镜像推送后停报）**。」
  ⇒ **W 段 B-3 造墙预授权即日起撤销**（见 §1 置顶行）；停点改为 **Z-0.6**。
...
- 【已作废·仅留追溯】原记：「- **停在哪：K-7.4**（用户发 `.国情`）→ K-7.5~K-7.7 → K-8 收尾 → 停报等裁决是否进 V-3。」
  ⇒ 停点已多次前移（K-7.4 → W-0.6 → Z-0.6），以 §1 置顶行为准。
- **停在哪：Z-0.6**（Z 段归档推送后停报；随后 Z-1 由用户执行视觉基准核对；
