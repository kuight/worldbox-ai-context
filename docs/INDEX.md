...
| 文件 | 用途 | 字节数 | 最后更新轮次 |
|---|---|---|---|
| `AGENT-CONTRACT.md` | Agent 行为契约与铁律（脱敏纪律、运行模型记录约定；每轮开工前必读） | 6585 | R4-G-fix |
| `DECISIONS.md` | 决策与排除项（ADR 风格，只增不删；D-001~D-025） | 22480 | R4-0 |
| `MEMORY.md` | 长期记忆，每轮累加的恢复依据（§1~§26，含组件哈希与实机结论） | 43060 | R3-V |
| `HANDOFF-R4.md` | R4 轮在途交接（R4-0/R4-1/R4-2a/R4-2b 进度与 D-1~D-4 补丁） | 14579 | R4-2b |
| `STATE-BRIEF.md` | 参谋侧恢复指挥速览（环境栈/可行性/风险，整份粘贴用） | 3323 | R0 |
| `RUNNER-HANDOFF.md` | 执行侧换对话恢复用（当前任务书/动过的文件/闸门） | 1948 | R3-W |
...
| `QQBOT-V1-ARCH.md` | QQ bot v1 架构稿（九模块/文法/账本/回执/待批） | 11846 | R3-V |
